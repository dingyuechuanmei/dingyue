...
class Pusher_EweiShopV2Page extends PluginWebPage
{
    ...
    public function main(){
        ...
        $sql = 'select p.id,p.title,p.pusher,p.like_count,p.ifshow,p.createtime,m.id mid,m.nickname,m.realname,m.avatar,m.mobile from '.tablename($this->tb_pusher).' p inner join '.tablename($this->tb_member).' m on(p.pusher = m.openid)';
    ...
    protected function post(){
        ...
        $category = pdo_fetchall('select id,category from '.tablename($this->tb_pusher_category).' where uniacid=:uniacid and passport=0',array(':uniacid'=>$_W['uniacid']));
        include $this->template('raise/pusher/post');
    }

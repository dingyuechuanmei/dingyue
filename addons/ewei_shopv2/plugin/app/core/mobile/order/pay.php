...
class Pay_EweiShopV2Page extends AppMobilePage
{
	public function main()
	{
		...
		if (!empty($set['pay']['wxapp']) && (0 < $order['price']) && $this->iswxapp) {
			...
			if (!is_error($res)) {
				...
				pdo_update('ewei_shop_order',['prepayid'=>$prepayid],['id'=>$orderid]);
			}
			else {
				$wechat['payinfo'] = $res;
			}
	...
	protected function success($orderid)
	{
        ...
        $child_order = [];
        if ($order['parentid'] == 0) {
            $child_order = pdo_fetchall("SELECT merchid FROM ".tablename('ewei_shop_order')." WHERE merchid != 0 AND parentid = :parentid", array(':parentid'=>$order['id']));
        }
        if (!empty($order['merchid'] || !empty($child_order))) {
            $child_order[]['merchid'] = $order['merchid'];
            foreach ($child_order as $v) {
                $mobile = pdo_getcolumn('ewei_shop_merch_user',array('id'=>$v['merchid']),'mobile');
                if (!empty($mobile)) {
                    $message = '您有新的订单,请及时登录商家后台处理';
                    m('common')->send_msg($mobile,$message);
                }
            }
        }

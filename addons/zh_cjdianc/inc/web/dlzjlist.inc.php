...
$pagesize=10;
$where=" where a.uniacid=:uniacid and a.type=1 and a.store_id=:store_id and a.pay_type=1 and a.state in (2,3,4,5,7,8,9,10)" ;
...
//总数统计
$sql2="select sum(money) as 'total_money', sum(discount) as 'discount',sum(ps_money) as ps_money from" . tablename("cjdc_order") ." as a".$where;
...
$ps_money=number_format($list2['ps_money']*$sys['ps_poundage']/100,2);
$fw_money= number_format($list4['poundage']/100*($list2['total_money']-$list3['tk_money']),2);
...
if(checksubmit('export_submit', true)) {
  $start=strtotime($_GPC['time']['start']);
  $end=strtotime($_GPC['time']['end']);
  $count = pdo_fetchcolumn("SELECT COUNT(*) FROM". tablename("cjdc_order")." WHERE type=1 and store_id={$storeid} and pay_type=1 and state in (2,3,4,5,7,8,9,10) and UNIX_TIMESTAMP(time) >='{$start}' and UNIX_TIMESTAMP(time)<='{$end}'");
  ...
  for ($j = 1; $j <= $pagesize; $j++) {
    $sql = "select * from " . tablename("cjdc_order")." WHERE type=1 and store_id={$storeid} and pay_type=1 and state in (2,3,4,5,7,8,9,10) and UNIX_TIMESTAMP(time) >='{$start}' and UNIX_TIMESTAMP(time)<='{$end}' ORDER BY id DESC limit " . ($j - 1) * 5000 . ",5000 ";

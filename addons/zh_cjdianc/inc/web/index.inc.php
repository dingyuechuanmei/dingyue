...
$jr_wxmoney=number_format($list2['total_money']-($list2['total_money']*$list4['poundage']/100)-($list2['ps_money']*$sys['ps_poundage']/100),2);
$jr_ddmoney=number_format($list3['total_money']-($list3['total_money']*$list4['poundage']/100)-($list3['ps_money']*$sys['ps_poundage']/100),2);
$jr_yemoney=number_format($list5['total_money']-($list5['total_money']*$list4['poundage']/100)-($list4['ps_money']*$sys['ps_poundage']/100),2);
...
//var_dump($list2);die;
$dnjr_wxmoney=number_format($list21['total_money']-($list21['total_money']*$list4['dn_poundage']/100),2);
$dnjr_ddmoney=number_format($list31['total_money']-($list31['total_money']*$list4['dn_poundage']/100),2);
$dnjr_yemoney=number_format($list51['total_money']-($list51['total_money']*$list4['dn_poundage']/100),2);
//今日订单统计
...
$ydjr_wxmoney=number_format($list61['total_money']-($list61['total_money']*$list4['dn_poundage']/100),2);
$ydjr_yemoney=number_format($list71['total_money']-($list71['total_money']*$list4['dn_poundage']/100),2);
...
//总营业额统计
//外卖在线总金额
$wmcost=pdo_get('cjdc_order', array('store_id'=>$storeid,'state '=>array(4,5,10),'pay_type'=>1,'type'=>1), array('sum(money) as total_money'));  
//外卖线下支付总金额
$xxcost=pdo_get('cjdc_order', array('store_id'=>$storeid,'state '=>array(4,5,10),'pay_type'=>4,'type'=>1), array('sum(money) as total_money'));
//外卖余额支付总金额
$wmyecost=pdo_get('cjdc_order', array('store_id'=>$storeid,'state '=>array(4,5,10),'pay_type'=>2,'type'=>1), array('sum(money) as total_money'));
//店内在线总金额
...
$zxsxf=($wmcost['total_money']*$list4['poundage']+$dnwmcost['total_money']*$list4['dn_poundage']+$dmfkcost['total_money']*$list4['dm_poundage']+$ydcost['total_money']*$list4['yd_poundage'])/100;
$zxcost=number_format($zxcost-$zxsxf,2);
$zxcost=$zxcost-$zxps;
//余额支付总数
$yecost=$wmyecost['total_money']+$dnyecost['total_money']+$dmxxcost['total_money']+$ydyecost['total_money'];
$yesxf=($wmyecost['total_money']*$list4['poundage']+$dnyecost['total_money']*$list4['dn_poundage']+$dmxxcost['total_money']*$list4['dm_poundage']+$ydyecost['total_money']*$list4['yd_poundage'])/100;
$yecost=number_format($yecost-$yesxf,2);
$yecost=$yecost-$yeps;
//货到付款支付总数
$xxcost=$xxcost['total_money']+$dnxxcost['total_money'];
$xxsxf=($xxcost['total_money']*$list4['poundage']+$dnxxcost['total_money']*$list4['dn_poundage'])/100;
$xxcost=number_format($xxcost-$xxsxf,2);
$xxcost=$xxcost-$hdfk;

//总数
$cost=number_format($zxcost+$yecost+$xxcost,2);

            ...
            for ($i = 2; $i <=  $num; $i++) { //$=2 第二行开始
               ...
               /* for ($j = 1; $j <= $data->sheets[0]['numCols']; $j++) {//获取某个单元格的数据
                ...
                //开始处理数据库
                   $insert['num'] = $row[1];
                   $insert['name'] = $row[2];
                   $insert['logo'] = $url.$row[3];
                   $insert['inventory'] = $row[4];
                   $insert['dn_money'] = $row[5];
                   $insert['sales'] = $row[6];
                   $insert['type'] = $row[7];
                   $insert['box_money'] = $row[8];
                   $insert['is_zp'] = $row[9];
                   $insert['is_show'] = $row[10];
                   $insert['details'] = $row[11];
                   $insert['type_id'] = $row[12];
                    $insert['money'] = $row[13];
                   $insert['money2'] = $row[14];

...
$url=substr($str,0,$n);
	...
	if($order['type']==1 and ($order['state']==1 || $order['state']==6)){//外卖
		...
		//file_get_contents("".$url."/app/index.php?i=".$order['uniacid']."&c=entry&a=wxapp&do=addintegral&m=zh_cjdianc&type=1&order_id=".$order['id']);//短信
	}
	...
	if($order['type']==3 and $order['yy_state']==1){//预约
		...
		//file_get_contents("".$url."/app/index.php?i=".$order['uniacid']."&c=entry&a=wxapp&do=addintegral&m=zh_cjdianc&type=4&order_id=".$order['id']);//短信
	}
